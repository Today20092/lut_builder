...
| Define full/legal-range semantics and verify black, middle grey, and white | Corrected. Endpoint semantics and all configured camera/target gamut excursions are covered and bounded to the selected range. | [Range regressions](../../tests/test_diagnostic_semantics.py), [issue #10](https://github.com/Today20092/lut_builder/issues/10) |
...
| Validate and dispatch every configured transfer function, including optional log output | Corrected. Catalog validation exercises configured camera decoders, and optional log-output generation dispatches the configured encoder. | [Dispatch regressions](../../tests/test_smoke.py), [issue #11](https://github.com/Today20092/lut_builder/issues/11) |
